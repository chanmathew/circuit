...
Schema: `packages/db/src/schema.ts`. Migrations: `packages/db/migrations/` (generated by Drizzle Kit only — never hand-write SQL).
...
For a clean dev DB: `pnpm --filter @circuit/db db:reset` (deletes `packages/db/.data/circuit-dev.db` and re-applies migrations). To wipe the **app** DB, quit the app and delete
`~/Library/Application Support/desktop/circuit.db` (and `-wal` / `-shm` if present), then restart — migrations run on launch.

Migrations are squashed for development (`0000_reflective_darkhawk.sql`, `0001_next_purple_man.sql`). When the baseline changes, wipe the app or dev DB and re-run migrations — no in-place upgrade path.
...
- **`db:migrate` / `db:reset` / `db:studio`** → need the **Node** build during the command (`pnpm rebuild better-sqlite3`), then restore Electron automatically (`pnpm --filter desktop rebuild:native` at the end; for Studio, that runs when you quit Studio)

If something still breaks (e.g. you ran `pnpm rebuild better-sqlite3` by hand), restore the build for the runtime you care about:
