...
**Goal:** Replace generic approval buttons with View + contextual proceed; move primary CTA to workflow panel.
...
**Documented MVP behavior:** Any composer message that does not end with `?` during `needs_review` triggers `requestPhaseRevision` and re-runs the phase. Questions continue through the normal chat harness.
